...
let handleMessageBus = (channel, message) => {
  log.debug("Received message " + message + " from channel " + channel);
...
let getSessionIdFromLobbyId = (lobbyId, redis) => {
  if (!(lobbyId in lobbies)) {
    redis.get("lobby:" + lobbyId).then((session) => {
      lobbies[lobbyId] = { id: session };
...
let getSession = (sessionId, redis) => {
  if (!(sessionId in sessions)) {
    let sessionKey = "session:" + sessionId;
    redis.get(sessionKey + "players").then((players) => {
      redis.get(sessionKey + "game").then((game) => {
        sessions[sessionId] = { players, game };
...
let createLobby = (json, redis, rsub) => {
  ...
  log.info("new lobby created,", lobbies[lobbyId]);
  log.info("new session created,", sessions[sessionId]);
  return {
...
let joinLobby = (json, redis, rsub, rpub) => {
  ...
  if (json.name) {
    ...
    log.info("joining session ", session);
    session.players = {
      ...session.players,
      name: "socketorsomething:" + name,
    };
    log.info(
      "Player " +
        name +
